	...
	ObjBool LibObjFile::obj_file_load(const string& objname)
	{
		...
		while (obj_stream.good())
		{
			...
			if (obj_flag == "v"){			// position flag
				...
				if (obj_save_mesh(positions, texcoords, normals, faces, groupname, mesh_))
				{
				...
				obj_parse_group(obj_stream, groupname);
			...
			{
			if (obj_save_mesh(positions, texcoords, normals, faces, last_groupname+usemtl, mesh_))
			{
			m_meshes.back().mtl_idx = mtl_index(usemtl);
		...
		//save mesh data
		if (obj_save_mesh(positions, texcoords, normals, faces, /*last_groupname+usemtl*/groupname, mesh_))
		{
			//m_meshes.back().mtl_idx = mtl_index(usemtl);
