...
bool SH_SCENE::GenerateDirectCoeffs(int numBands, int numSamples, SAMPLE * samples)
{
	...
	if(!regenerateCoeffs)
	{
		for(int i=0; i<numObjects; ++i)
		{
			...
			for(int j=0; j<numVertices; ++j)
			{
				...
				for(int t=0; t<numFunctions; ++t)
					inFile >> currentVertex.unshadowedCoeffs[t];
				for(int t=0; t<numFunctions; ++t)
					inFile >> currentVertex.shadowedCoeffs[t];
			}
		}
	...
	outFile << numBands << std::endl;
	outFile << numSamples << std::endl;

	std::string filename = "model/data-raytracing/";
	//Loop through the vertices and save the coefficients for each
	for(int i=0; i<numObjects; ++i)
	{
		SH_OBJECT * currentObject=objects[i];

		std::ofstream outPerFile(filename + currentObject->name + ".txt", std::ios::out /*| std::ios::binary*/ | std::ios::trunc);

		outPerFile<<numBands<<std::endl;
		outPerFile<<numSamples<<std::endl;
		...
		for(int j=0; j<numVertices; ++j)
		{
			...
			numFunctions*sizeof(double));*/
			for(int t=0; t<numFunctions-1; ++t){
				outFile << currentVertex.unshadowedCoeffs[t] << "\t";
				outPerFile << currentVertex.unshadowedCoeffs[t] << "\t";
			}
			outFile << currentVertex.unshadowedCoeffs[numFunctions-1] << std::endl;
			outPerFile << currentVertex.unshadowedCoeffs[numFunctions-1] << std::endl;

			for(int t=0; t<numFunctions-1; ++t){
				outFile << currentVertex.shadowedCoeffs[t] << "\t";
				outPerFile << currentVertex.shadowedCoeffs[t] << "\t";
			}
			outFile << currentVertex.shadowedCoeffs[numFunctions-1] << std::endl;
			outPerFile << currentVertex.shadowedCoeffs[numFunctions-1] << std::endl;
		}
...
bool SH_SCENE::GenerateMyDirectCoeffs(int numBands, int numSamples, SAMPLE * samples)
{
	const int numFunctions=numBands*numBands;
	...
	for(int i=0; i<numObjects; ++i)
	{
		SH_OBJECT * currentObject=m_objects[i];
		const int numVertices=currentObject->vertices.size();
	...
	for(int i=0; i<numObjects; ++i)
	{
		...
		//Save the coefficients to a file
		std::ofstream outFile(filename + currentObject->name + ".txt", std::ios::out /*| std::ios::binary*/ | std::ios::trunc);
